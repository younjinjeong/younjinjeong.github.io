...
유효환 에러 상수는 모두 양수이다. <errno.h> 헤더 파일에서는 *errno*의 값이 될 수 있는 양수로 되어있는 각각의 에러 상수를 심볼릭 이름으로 정의하고 있다.  

모든 에러의 이름은 POSIX.1에 정의되어 있으며, 각각의 에러는 *EAGAIN*과 *EWOULDBLOCK* 를 제외한 반드시 고유한 값을 가져야 한다. 리눅스에서는 이 두개는 모든 아키텍처에서 동일한 값을 가진다. 

에러 상수는 매우 광범위한 UNIX 시스템과 리눅스가 지원하는 다양한 아키텍처에서 넓게 사용되는 상징적 이름과 매칭된다. 따라서 특정 에러에 대흥하는 상수는 아래에 별도로 명시하지 않는다. [perror(3)](https://man7.org/linux/man-pages/man3/perror.3.html)와 [strerror(3)](https://man7.org/linux/man-pages/man3/strerror.3.html) 함수들은 이 에러 이름들을 식별 가능한 에러 메세지로 변환하는데 사용된다. 

리눅스 시스템에서는 아래에 나열된 모든 심볼릭 에러 리스트를 전부 제공할 가능성이 있으며 에러 이름과 그에 상응하는 에러 상수를 *errno(1)* 명령어를 사용하여 조회가 가능하다. (*errno* 명령어는 *moreutils* 패키지에 포함되어 있다)
...
*errno(1)* 명령어는 특정 에러 상수와 이름을 검색하는데 사용할 수도 있으며, 이를 통해 해당 에러에 대한 설명도 함께 조회할 수 있다. 아래의 예제를 살펴보자. 
...
POSIX 스레드 API는 에러 발생시 *errno*에 값을 설정하지 않는다. 대신 호출 실패가 발생하면 함수 수행의 리턴값으로 에러 코드를 리턴한다. 이떄 리턴하는 에러 상수는 *errno*에서 사용되는 것과 동일한 의미를 가진다. 
